...
{
    public class MaintainanceSubServiceDAL
    {
        public List<SubServiceObject> GetSubServiceUnit(int campanyId)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(ConString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("mt_SP_SubService", conn))
                    {
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        SubServiceObject.BusinessName = dr["BusinessName"].ToString();
                        SubServiceObject.SubServiceID = int.Parse(dr["SubServiceID"].ToString());
                        SubServiceObject.serviceCode = (dr["serviceCode"].ToString());
                        ...
                        if (ds.Tables.Count > 0) {
                            ...
                            if (dt.Rows.Count > 0) {
                                foreach(DataRow dr in dt.Rows)
                                {
                                    ...
                                    serviceUnitObj.ServiceName = dr["SubServiceDescription"].ToString();
