...
namespace HelpDeskServices.DataAccessLayer.Maintenance_mgmt.PM_by_system
{
    public class CreateplanDAL
    {
        ...
        public IEnumerable<object> getcreatedetails(allassetsreq req)
        {
            ...
            try
            {
                ...
                var parameters = new SqlParamDefinition[]
                {
                  ...
                  new SqlParamDefinition("@AssetRoute",  req.Assetroute),
                   };
                // SqlDataAdapter da = new SqlDataAdapter();
                DataSet ds = spexc.ExecuteSelectProcedure("[HD_SP_Pmbysystem]", parameters);
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    var convertedList = (from rw in dt.AsEnumerable()
                                         select new
                                         {
                                             ...
                                             Frequency = Convert.ToString(rw["Frequency"]),
                                         }).ToList();
        ...
        public IEnumerable<object> getroutemappingdetails(allassetsreq req)
        {
            RaiseServiceDAL format = new RaiseServiceDAL();
            List<object> responce = new List<object>();
            try
            {
                ...
                var parameters = new SqlParamDefinition[]
                {
                  new SqlParamDefinition("@Fcase",  10),
                  new SqlParamDefinition("@location",  req.Locations),
                  new SqlParamDefinition("@Companyid",  req.Companyid),
                  new SqlParamDefinition("@zone",  req.Zone),
                  new SqlParamDefinition("@building",  req.Building),
        ...
        //            using (SqlCommand cmd = new SqlCommand("[HD_SP_Pmbysystem]", conn))
        //            {
        //                // SqlDataAdapter da = new SqlDataAdapter();
        ...
        //                cmd.Parameters.AddWithValue("@uniqueAssetId", req.uniqueAssetId);
        ...
        public IEnumerable<object> getlocationbyassetSubsys(int companyId, string Subsys)
        {
                    ...
                    using (SqlCommand cmd = new SqlCommand("Hd_sp_Multidropdowns", conn))
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            var convertedList = (from rw in dt.AsEnumerable()
                                                 ...
                                                 {
                                                     ID = Convert.ToString(rw["ID"]),
                                                     SiteName = Convert.ToString(rw["SiteCode"]) + " " + Convert.ToString(rw["SiteName"]),
                                                 }).ToList().Distinct();
                        ...
                        for (int t = 0; t < frqnames.Length; t++)
                        {
                            ...
                            if (frqnames[t]== "Monthly") {
                                for (int i=0; i<12;i++)
                                {
                                    ...
                                    CreatePmsysplanchild(createplan, 1, Convert.ToInt32(id), item.FromDate, item.Todate, addeddate);
        ...
        public DataTable getassetdetailsbyid(int Id)
        {
            List<object> baseModel = new List<object>();
            DataTable dt = new DataTable();
        ...
        public string CreatePmsysplanchild(plan req, int companyid, int id, string fromdate, string todate, DateTime addeddate)
        {
        ...
        public Hd_Responce Addchilddats(List<listofresourcesandspares> request)
        {
            Hd_Responce responce = new Hd_Responce();
            try
            {
                foreach (var item in request)
                {
                    ...
                    string tools = "";
                    string consumbels = "";
                    if (item.ResourcesList.Count > 0)
                    {
                        resourcesdat = CreateWoResorce(item.ResourcesList, item.WOId, item.reqid, item.CompanyId);
                    }
                    //string Spares = CreateWOSpares(request.SparesList, request.WOId, request.CompanyId);
                    if (item.SparesList.Count > 0) { Spares = CreateWOSpares(item.SparesList, item.WOId, item.reqid, item.CompanyId); }

                    if (item.ToolsList.Count > 0)
                    { tools = CreateWOtools(item.ToolsList, item.WOId, item.reqid, item.CompanyId); }
                    if (item.ConsumabelsList.Count > 0)
                    { consumbels = CreateWOConsumbels(item.ConsumabelsList, item.WOId, item.reqid, item.CompanyId); }


                    if (resourcesdat == "Saved")
                    {
                        responce.Status = "200";
                        responce.Message = "Success";
                        responce.Result = null;
              ...
              string status=  updatesheduleplan(request[0].slectedassetslist);
                if (status == "Updated")
                {
                    responce.Status = "200";
                    responce.Message = "Success";
        ...
        private string updatesheduleplan(List<Selectedassets> req)
        {
            string status = "";
            ...
            foreach (var item in req)
            {
                ...
                var parameters = new SqlParamDefinition[]
                   {
                  new SqlParamDefinition("@Fcase",  11),
                  new SqlParamDefinition("@Id",  item.childid),
                  new SqlParamDefinition("@AssetType",item.slotid+1),
                  new SqlParamDefinition("@startdate",item.startdate),
                  new SqlParamDefinition("@enddate",item.enddate),
                  new SqlParamDefinition("@condition",item.bypassstatus),
                   };
                DataSet ds = spexc.ExecuteSelectProcedure("[HD_SP_Pmbysystem]", parameters);
                DataTable dt = ds.Tables[0];
                if (dt.Rows.Count > 0)
        ...
        public string CreateWoResorce(List<Resources> request, int Id, int cpid, int CompanyId)
        {
            string result = "";
            ...
            try
            {
                foreach (var data in request)
                {
                    ...
                    {
                  new SqlParamDefinition("@Fcase", 1),
                  new SqlParamDefinition("@Id", ""),
                  new SqlParamDefinition("@ResourceId", data.resourceId),
                  new SqlParamDefinition("@Code", data.Code),
                  new SqlParamDefinition("@Name", data.Name),
                  new SqlParamDefinition("@Fromdate", data.Fromdate),
                  new SqlParamDefinition("@Todate", data.Todate),
                  new SqlParamDefinition("@CompanyId", CompanyId),
                  new SqlParamDefinition("@TimeTaken", data.TimeTaken),
                  new SqlParamDefinition("@WorkOrderId", Id),
                  new SqlParamDefinition("@cpid", cpid),
                  new SqlParamDefinition("@IsDone", false),
                  new SqlParamDefinition("@Remarks", data.Remarks),
                    };
                    DataSet ds = spexc.ExecuteSelectProcedure("[HD_SP_PmsysResource_spares_tool_consm]", parameters);
                    DataTable dt = ds.Tables[0];
        ...
        public string CreateWOSpares(List<ToolsSpresConsumables> request, int Id, int cpid, int CompanyId)
        {
            string result = "";
            ...
            try
            {
                ...
                foreach (var data in request)
                {
                  ...
                  new SqlParamDefinition("@Id", ""),
                  new SqlParamDefinition("@ItemId", data.ItemId),
                  new SqlParamDefinition("@Code", data.code),
                  new SqlParamDefinition("@Name", data.Name),
                  new SqlParamDefinition("@Category", data.Category),
                  new SqlParamDefinition("@SubCategory", data.SubCategory),
                  new SqlParamDefinition("@Quantity", data.ReqQuantity),
                  new SqlParamDefinition("@ConsumeQty", data.ConsumeQty),
                  new SqlParamDefinition("@InventoryType", data.InventoryType),
                  new SqlParamDefinition("@Services", data.ServiceDescription),
                  new SqlParamDefinition("@ComapanyId", CompanyId),
                  new SqlParamDefinition("@WorkOrderId", Id),
                  new SqlParamDefinition("@cpid", cpid),
                  new SqlParamDefinition("@IsDone", false),
                  new SqlParamDefinition("@Remarks", data.Remarks),
        ...
        public string CreateWOtools(List<ToolsSpresConsumables> request, int Id, int cpid, int CompanyId)
        {
            string result = "";
        ...
        public string CreateWOConsumbels(List<ToolsSpresConsumables> request, int Id, int cpid, int CompanyId)
        {
        ...
        public Hd_Responce getchildcounts(int Id)
        {
            ...
            try
            {
                var parameters = new SqlParamDefinition[]
              {
                  new SqlParamDefinition("@Fcase",  13),
                  new SqlParamDefinition("@CPID",  Id)
              };
                DataSet ds = spexc.ExecuteSelectProcedure("[HD_SP_Pmbysystem]", parameters);
                DataTable dt = ds.Tables[0];
                if (dt.Rows.Count > 0)
                {
                    var convertedList = (from rw in dt.AsEnumerable()
                                         select new
                                         {
                                             ...
                                             sitename = Convert.ToString(rw["sitename"]),
                                             zone = Convert.ToString(rw["zonename"]),
                                             slotid = Convert.ToString(rw["slotid"]),
                                             startdate = Convert.ToString(rw["startdate"]),
                                             cpid = Convert.ToString(rw["cpid"]),
                                             enddate = Convert.ToString(rw["enddate"]),
                                             CreatedDate = Convert.ToString(rw["CreatedDate"]),
                                             status = Convert.ToString(rw["status"]),
                                             Assetcount = Convert.ToString(rw["Assetcount"]),
                                             resourcecount = Convert.ToString(rw["resourcecount"]),
                                             sparescount = Convert.ToString(rw["sparescount"]),
                                             toolscount = Convert.ToString(rw["toolscount"]),
                                             COnsumcount = Convert.ToString(rw["COnsumcount"]),
                                             formatexpressiom = Convert.ToString(rw["formantname"]) + Convert.ToString(rw["date"]),
        ...
        public Hd_Responce GetprintWOList(allassetsreq req)
        {
            RaiseServiceDAL format = new RaiseServiceDAL();
            Hd_Responce responce = new Hd_Responce();
            try
            {
                ...
                var parameters = new SqlParamDefinition[]
                {
                  new SqlParamDefinition("@Fcase",  16),
                  new SqlParamDefinition("@location",  req.Locations),
                  new SqlParamDefinition("@CPID",  req.id),
                  new SqlParamDefinition("@Companyid",  req.Companyid),
                  new SqlParamDefinition("@zone",  req.Zone),
        ...
        public Hd_Responce getchilddata(int Id)
        {
            ...
            try
            {
                var parameters = new SqlParamDefinition[]
              {
                ...
                DataSet ds = spexc.ExecuteSelectProcedure("[HD_SP_PlanChilds]", parameters);
                DataTable dt = ds.Tables[0];
                if (dt.Rows.Count > 0)
                {
                    var convertedList = (from rw in dt.AsEnumerable()
                                         select new
                                         {
                                             ...
                                             childId = Convert.ToString(rw["childid"]),
                                             assertid = Convert.ToString(rw["assertid"]),
                                             hours = Convert.ToString(rw["TLHours"]),
                                             assertid1 = Convert.ToString(rw["assertid"]),
                                             site = Convert.ToString(rw["sitename"]),
                                             zone = Convert.ToString(rw["zonename"]),
                                             building = Convert.ToString(rw["buildingname"]),
                                             floor = Convert.ToString(rw["floorname"]),
                                             ...
                                             service = Convert.ToString(rw["ServiceDescription"]),
                                             system = Convert.ToString(rw["System"]),
                                             Routename = Convert.ToString(rw["Routename"]),
                                             Status = Convert.ToString(rw["Status"]),
                                             createddate = Convert.ToString(rw["CreatedDate"]),
                                             formatexpressiom = Convert.ToString(rw["formantname"]) + Convert.ToString(rw["date"]),
                                             Frequency = (rw["M"].Equals(DBNull.Value) ? "" : "Monthly,") + (rw["Q"].Equals(DBNull.Value) ? "" : "Quarterly,") + (rw["S"].Equals(DBNull.Value) ? "" : "SemiAnnually,") + (rw["A"].Equals(DBNull.Value) ? "" : "Annually,"),
                                             Task_A_Id = (rw["TLM-A"].Equals(DBNull.Value) ? "" : rw["TLM-A"]),
                                             Task_M_Id = (rw["TLM-M"].Equals(DBNull.Value) ? "" : rw["TLM-M"]),
                                             Task_Q_Id = (rw["TLM-Q"].Equals(DBNull.Value) ? "" : rw["TLM-Q"]),
                                             Task_S_Id = (rw["TLM-S"].Equals(DBNull.Value) ? "" : rw["TLM-S"]),
                                             task_id = (rw["TLM-S"].Equals(DBNull.Value) ? (rw["TLM-A"].Equals(DBNull.Value) ? (rw["TLM-M"].Equals(DBNull.Value) ? (rw["TLM-Q"].Equals(DBNull.Value) ? "NA" : rw["TLM-Q"]) : rw["TLM-M"]) : rw["TLM-A"]) : rw["TLM-S"])

                                         }).Distinct().ToList();


                    var getlistfortasklist = (from rw in dt.AsEnumerable()
                                              select new
                                              {
                    ...
                    if (getlistfortasklist != null)
                    {
                        List<object> basetasklist = new List<object>();

                        foreach (var item in getlistfortasklist)
                        {
                            if (Convert.ToString(item.Task_A_Id) != "")
                            {
                                var data = getTasklist(Convert.ToString(item.Task_A_Id), Id);

                                basetasklist.Add(data);
                            }
                            if (Convert.ToString(item.Task_M_Id) != "")
                            {
                                var data = getTasklist(Convert.ToString(item.Task_M_Id), Id);
                                basetasklist.Add(data);
                            }
                            if (Convert.ToString(item.Task_Q_Id) != "")
                            {
                                var data = getTasklist(Convert.ToString(item.Task_Q_Id), Id);
                                basetasklist.Add(data);
                            }
                            if (Convert.ToString(item.Task_S_Id) != "")
                            {
                                var data = getTasklist(Convert.ToString(item.Task_S_Id), Id);
                                basetasklist.Add(data);
                            }
                        }

                        var childdat = getchilddetailsbyid(Id);


                        baseresponce.Add(convertedList);
                        baseresponce.Add(basetasklist);
                        baseresponce.Add(childdat);

                        responce.Status = "200";
                        responce.Message = "Success";
                        responce.Result = baseresponce;
                    }
                    else
                    {
                        responce.Status = "-100";
        ...
        private object getTasklist(string TLcode, int Id)
        {
            ...
            try
            {
                var parameters = new SqlParamDefinition[]
              {
                  ...
                  new SqlParamDefinition("@Id",  TLcode)
              };
                DataSet ds = spexc.ExecuteSelectProcedure("[HD_SP_PlanChilds]", parameters);
                DataTable dt = ds.Tables[0];
                if (dt.Rows.Count > 0)
                {
                    var convertedList = (from rw in dt.AsEnumerable()
                                         select new
                                         {
                                             Id = Id,
                                             TLCode = Convert.ToString(rw["TLCode"]),
                                             TLName = Convert.ToString(rw["TLName"]),
                                             TLHours = Convert.ToString(rw["Hours"]),
                                             TaskCategory = Convert.ToString(rw["SubTaskName"]),
                                             TaskChecklist = Convert.ToString(rw["TaskDesc"]),
        ...
        private object getslotlist(int Id)
        {
                ...
                var parameters = new SqlParamDefinition[]
              {
                ...
                if (dt.Rows.Count > 0)
                {
                    var convertedList = (from rw in dt.AsEnumerable()
                                         ...
                                         {
                                             slotid = Convert.ToString(rw["slotid"]),
                                             formantname = Convert.ToString(rw["formantname"]),
        ...
        private List<object> getchilddetailsbyid(int Id)
        {
            List<object> baseresponce = new List<object>();
            try
            {
                var parameters = new SqlParamDefinition[]
              {
                ...
                DataSet ds = spexc.ExecuteSelectProcedure("[HD_SP_PlanChilds]", parameters);
                DataTable resorse = ds.Tables[0];
                DataTable spares = ds.Tables[1];
                DataTable tools = ds.Tables[2];
                DataTable consumble = ds.Tables[3];
                if (resorse.Rows.Count > 0)
                {
                    var Resourcelist = (from rw in resorse.AsEnumerable()
                                        select new
                                        {

                                            ResourceID = Convert.ToString(rw["ResourceId"]),
                                            ResourceName = Convert.ToString(rw["Name"]),
                                            Code = Convert.ToString(rw["Code"]),
                                            FromDate = Convert.ToString(rw["Fromdate"]),
                                            ToDate = Convert.ToString(rw["Todate"]),
                                            placeorderId = Convert.ToString(rw["placeorderId"]),

                                        }).Distinct().ToList();

                    var Spareslist = (from rw in spares.AsEnumerable()
                                      select new
                                      {
                                          ItemCode = Convert.ToString(rw["Code"]),
                                          ItemDesc = Convert.ToString(rw["Name"]),
                                          ReqQty = Convert.ToString(rw["Quantity"]),
                                          ConsumedQty = Convert.ToString(rw["ConsumeQty"]),
                                          placeorderId = Convert.ToString(rw["placeorderId"]),

                                      }).Distinct().ToList();

                    var Toolslist = (from rw in tools.AsEnumerable()
                                     select new
                                     {
                                         ItemCode = Convert.ToString(rw["Code"]),
                                         ItemDesc = Convert.ToString(rw["Name"]),
                    ...
                    var consumblelist = (from rw in consumble.AsEnumerable()
                                         select new
                                         {
                                             ItemCode = Convert.ToString(rw["Code"]),
                                             ItemDesc = Convert.ToString(rw["Name"]),
                    ...
                    baseresponce.Add(Resourcelist);
                    baseresponce.Add(Spareslist);
                    baseresponce.Add(Toolslist);
                    baseresponce.Add(consumblelist);
        ...
        public Hd_Responce getchilddataforupdate(int Id)
        {
                ...
                if (dt.Rows.Count > 0)
                {
                    var convertedList = (from rw in dt.AsEnumerable()
                                         select new
                                         {
                                             ...
                                             wostatus = Convert.ToString(rw["wostatus"]),
                                             Remarks = Convert.ToString(rw["Remarks"]),
                                             Signature1 = Convert.ToString(rw["Signature1"]),
                                             Signature2 = Convert.ToString(rw["Signature2"]),
                                             Signature3 = Convert.ToString(rw["Signature3"]),
                                             ExecutionEndDate = Convert.ToString(rw["ExecutionEndDate"]),
                                             ExecutionStartDate = Convert.ToString(rw["ExecutionStartDate"]),
                                         }).Distinct().ToList();
                    ...
                    if (getlistfortasklist != null)
                    {
                        ...
                        var childdat = getchilddetailsbyidforupdate(Id);


                        var slotlist = getslotlist(Id);

                        baseresponce.Add(convertedList);
                        baseresponce.Add(basetasklist);
                        baseresponce.Add(childdat);
                        baseresponce.Add(slotlist);
        ...
        private List<object> getchilddetailsbyidforupdate(int Id)
        {
            List<object> baseresponce = new List<object>();
            try
            {
                ...
                if (resorse.Rows.Count > 0)
                {
                    var Resourcelist = (from rw in resorse.AsEnumerable()
                                        select new
                                        {
                                            ...
                                            Fromdate = Convert.ToString(rw["Fromdate"]),
                                            Todate = Convert.ToString(rw["Todate"]),
                                            // Id = Convert.ToString(rw["placeorderId"]),
                                            Id = Convert.ToString(rw["id"]),
                    ...
                    var Spareslist = (from rw in spares.AsEnumerable()
                                      select new
                                      {
                                          code = Convert.ToString(rw["Code"]),
                                          Name = Convert.ToString(rw["Name"]),
                                          ReqQuantity = Convert.ToString(rw["Quantity"]),
                                          ConsumeQty = Convert.ToString(rw["ConsumeQty"]),
                                          Category = Convert.ToString(rw["Category"]),
                                          SubCategory = Convert.ToString(rw["SubCategory"]),
                                          Id = Convert.ToString(rw["id"]),
                                          ItemId = Convert.ToString(rw["id"]),
                                          InventoryType = Convert.ToString(rw["InventoryType"]),
                                      }).Distinct().ToList();
        ...
        public Hd_Responce getTasklistForschedule(int Id)
        {
            ...
            try
            {
                var parameters = new SqlParamDefinition[]
              {
                  new SqlParamDefinition("@Fcase",  18),
                  new SqlParamDefinition("@Id",  Id)
              };
                DataSet ds = spexc.ExecuteSelectProcedure("[HD_SP_Pmbysystem]", parameters);
                DataTable dt = ds.Tables[0];
                if (dt.Rows.Count > 0)
                {
                    var convertedList = (from rw in dt.AsEnumerable()
                                         select new
                                         {
                                             Id = Convert.ToString(rw["CPID"]),
                                             TLHours = Convert.ToString(rw["Hours"]),
                                             Assetcount = Convert.ToString(rw["Assetcount"]),
                                             //TLRemarks = Convert.ToString(rw["TLDescription"]),
                                             TLName = Convert.ToString(rw["TLName"]),
                                             TLCode = Convert.ToString(rw["TLCode"]),
                                             Totalduration = getdatetime(Convert.ToString(rw["Assetcount"]), Convert.ToString(rw["Hours"])),
        ...
        int calculateTimeInMinutes(string currentTime)
        {
        ...
        string getdatetime(string assetcount, string Hour)
        {
            String str = Hour;
            int count = Convert.ToInt32(assetcount);
            ...
            {
                int Minutes = calculateTimeInMinutes(str);
                int totalmin = count * Minutes;
                totaltime = string.Format("{0:00} hours:{1:00} min", totalmin / 60, totalmin % 60);
            }
            else
            {
                int HoursInminutes = (Convert.ToInt32(str) % 60) * 60;
                int totalmin = count * HoursInminutes;
                totaltime = string.Format("{0:00} hours:{1:00} min", totalmin / 60, totalmin % 60);
            }
        ...
        public Hd_Responce CheckWOHoliday(string date, int ShiftId)
        {
            DateTime req_date = Convert.ToDateTime(date);
            Hd_Responce responce = new Hd_Responce();
            List<object> baseresponce = new List<object>();
            try
            {
              ...
              {
                  new SqlParamDefinition("@fcase",  1),
                  new SqlParamDefinition("@date",  req_date),
                  new SqlParamDefinition("@ShiftId",  ShiftId),
              };
                DataSet ds = spexc.ExecuteSelectProcedure("[Hd_Sp_Getholidayshiftlist]", parameters);
                DataTable holidaytable = ds.Tables[0];
                DataTable Shifttable = ds.Tables[1];
                int hoildaycount = 0;
                int count = 0;
                ...
                {
                    //default locale
                    string name = req_date.DayOfWeek.ToString();
                    var shiftdetailslist = (from rw in Shifttable.AsEnumerable()
                                            where Convert.ToString(rw["Weekoff"]) != ""
                                            select new
                                            {
                                                off = Convert.ToString(rw["Weekoff"]),

                                            }).FirstOrDefault();

                    if (shiftdetailslist != null)
                    {
                        string[] days = shiftdetailslist.off.Split(',');
                ...
                if (hoildaycount > 0 || count > 0)
                {
                    responce.Status = "200";
                    responce.Message = "Leave";
                    responce.Result = true;
                }
                else
                {
                    responce.Status = "200";
                    responce.Message = "No Leave";
        ...
        public Hd_Responce UpdatePmBySYchilddats(List<listofresourcesandspares> request)
        {
            Hd_Responce responce = new Hd_Responce();
            try
            {
                foreach (var item in request)
                {
                    ...
                    if (item.ResourcesList.Count > 0)
                    {
                        resourcesdat = UpdateWoResorce(item.ResourcesList, item.WOId, item.reqid, item.CompanyId);
                    }
                    //string Spares = CreateWOSpares(request.SparesList, request.WOId, request.CompanyId);
                    if (item.SparesList.Count > 0) { Spares = UpdateWOSpares(item.SparesList, item.WOId, item.reqid, item.CompanyId); }

                    if (item.ToolsList.Count > 0)
                    { tools = UpdateWOtools(item.ToolsList, item.WOId, item.reqid, item.CompanyId); }
                    if (item.ConsumabelsList.Count > 0)
                    { consumbels = UpdateWOConsumbels(item.ConsumabelsList, item.WOId, item.reqid, item.CompanyId); }


                    if (resourcesdat == "Saved"|| resourcesdat == "updated")
                    {
                        responce.Status = "200";
                        responce.Message = "Success";
                        responce.Result = null;
                ...
                updatePMBYSYSWO(request[0].slectedassetslist);
        ...
        private string updatePMBYSYSWO(List<Selectedassets> req)
        {
            foreach (var item in req)
            {
                ...
                var parameters = new SqlParamDefinition[]
                   {
                  new SqlParamDefinition("@Fcase",  19),
                  new SqlParamDefinition("@Id",  item.childid),
                  new SqlParamDefinition("@status",item.status),
                  new SqlParamDefinition("@startdate",item.executstartdate),
                  new SqlParamDefinition("@enddate",item.executendadate),
                  new SqlParamDefinition("@Remarks",item.remarks),
                  new SqlParamDefinition("@signature1",item.signature1),
                  new SqlParamDefinition("@signature2",item.signature2),
                  new SqlParamDefinition("@signature3",item.signature3),
                   };
                DataSet ds = spexc.ExecuteSelectProcedure("[HD_SP_Pmbysystem]", parameters);
                DataTable dt = ds.Tables[0];
                if (dt.Rows.Count > 0)
        ...
        public string UpdateWoResorce(List<Resources> request, int Id, int cpid, int CompanyId)
        {
            string result = "";
        ...
        public string UpdateWOSpares(List<ToolsSpresConsumables> request, int Id, int cpid, int CompanyId)
        {
            string result = "";
            ...
            try
            {
                ...
                foreach (var data in request)
                {
                    if (data.Id > 0)
                    {
                        var parameters = new SqlParamDefinition[]
                                           {
                  new SqlParamDefinition("@Fcase", 6),
                  new SqlParamDefinition("@Id", data.Id),
                  new SqlParamDefinition("@ItemId", data.ItemId),
                  new SqlParamDefinition("@Code", data.code),
                  new SqlParamDefinition("@Name", data.Name),
        ...
        public string UpdateWOtools(List<ToolsSpresConsumables> request, int Id, int cpid, int CompanyId)
        {
            string result = "";
        ...
        public string UpdateWOConsumbels(List<ToolsSpresConsumables> request, int Id, int cpid, int CompanyId)
        {
        ...
        public Hd_Responce updateWoPmBySys(List<closereq> req)
        {
            ...
            try
            {
                    ...
                    var parameters = new SqlParamDefinition[]
              {
                  new SqlParamDefinition("@Fcase",  2),
                  new SqlParamDefinition("@Id",  item.Id),
                  new SqlParamDefinition("@statusid",  item.Status),
                  new SqlParamDefinition("@remarks",  item.Remarks)
              };
                    DataSet ds = spexc.ExecuteSelectProcedure("[Hd_sp_Pmbysys_Complete]", parameters);
                    DataTable dt = ds.Tables[0];
        ...
        public Hd_Responce GetpmbySYSWoCloselist(allassetsreq req)
        {
            RaiseServiceDAL format = new RaiseServiceDAL();
            Hd_Responce responce = new Hd_Responce();
            try
        ...
        public Hd_Responce getclosedetailsbyplan(int Id)
        {
 ...
 public Hd_Responce GetpmbySYSWoreshedulelist(allassetsreq req)
        {
            RaiseServiceDAL format = new RaiseServiceDAL();
            Hd_Responce responce = new Hd_Responce();
            try
        ...
        public Hd_Responce reshedulePmbysyschilddats(List<listofresourcesandspares> request)
        {
            Hd_Responce responce = new Hd_Responce();
            try
            {
                ...
                reshedulePMBYSYSWO(request[0].slectedassetslist);
        ...
        private string reshedulePMBYSYSWO(List<Selectedassets> req)
        {
            foreach (var item in req)
            {
                ...
                var parameters = new SqlParamDefinition[]
                   {
                  new SqlParamDefinition("@Fcase",  21),
                  new SqlParamDefinition("@Id",  item.childid),
                  new SqlParamDefinition("@startdate",item.executstartdate),
                  new SqlParamDefinition("@Remarks",item.remarks),
                   };
